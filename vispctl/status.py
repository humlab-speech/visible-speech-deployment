"""Display helpers for 'visp.py status' — quadlet unit table, container list, network list."""
...
def show_quadlet_table(
    ...
    """Print the quadlet unit table showing each service's install state."""
    from .quadlets import get_quadlet_drift, get_quadlets_dir

    print(color("=== Quadlet Units ===", Colors.CYAN))
    ...
    for svc in runtime_services:
        unit_path = systemd_dir / svc.file

        if svc.file in not_installed_files:
