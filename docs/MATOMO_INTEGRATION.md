...
### Phase 3: Register in `visp.py`

Add to the `SERVICES` list in `visp.py`:
...
expanded during `visp.py install`.
...
These are created automatically by `./visp.py install`
...
- [x] Add `matomo-db` and `matomo` to `SERVICES` in `visp.py`
...
Use the credentials auto-generated during `./visp.py install`:
