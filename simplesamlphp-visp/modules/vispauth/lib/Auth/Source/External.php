...
namespace SimpleSAML\Module\vispauth\Auth\Source;
 ...
 * 3. Copy this file and modules/vispauth/www/resume.php to their corresponding
 *    location in the new module.
 * 4. Replace all occurrences of "vispauth" in this file and in resume.php with the name of your module.
...
class External extends \SimpleSAML\Auth\Source
{
     ...
     */
    public const AUTHID = 'SimpleSAML\Module\vispauth\Auth\Source\External.AuthId';
	...
	//Here we (might) have an active PHP session with SSP, but we wish to get the session attributes from the visp webapi session, so we switch session
	session_destroy();
	...
	*/
	...
	//VISP attributes retrieved, so switch back to the SSP session
        session_destroy();
    ...
    public function authenticate(&$state)
    {
         ...
         */
        $state['vispauth:AuthID'] = $this->authId;
         ...
         */
        $stateId = Auth\State::saveState($state, 'vispauth:External');
         ...
         */
        $returnTo = Module::getModuleURL('vispauth/resume.php', [
            'State' => $stateId,
        ]);
        ...
        $authPage = "https://".getenv("BASE_DOMAIN")."/DS/Login";
    ...
    public static function resume()
    {
         ...
         */
        /** @var array $state */
        $state = Auth\State::loadState($_REQUEST['State'], 'vispauth:External');

        /*
         * Now we have the $state-array, and can use it to locate the authentication
         * source.
         */
        $source = Auth\Source::getById($state['vispauth:AuthID']);
        if ($source === null) {
            /*
